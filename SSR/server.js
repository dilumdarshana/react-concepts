...
/**
  ...
  It dynamically selects the appropriate SSR render function and template based on the environment 
  (development or production) and serves the rendered HTML to clients upon request.
...
const staticFileExtensions = ['.js', '.css', '.jpg', '.png', '.ico', '.svg', '.woff2'];

// Serve HTML
// "*home" is Express 5.x syntax for matching all routes
app.use('*all', async (req, res, next) => {
  try {
    if (staticFileExtensions.some(ext => req.originalUrl.endsWith(ext))) {
      return next(); // Skip SSR handling for service worker
    }
